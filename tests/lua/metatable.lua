...
do
    template_test_function = function(func_name, wrong_arg_num, expected_type, ...)
        local args = {...}
    ...
    local supports_stable = { ["effil.table"] = true }
    local supports_table_and_stable = { ["table"] = true, ["effil.table"] = true }
    local supports_all = setmetatable({}, {__index = function() return true end})

    local funcs_to_test = {
        -- function name = { [param index] = { list of supported types } }
        setmetatable = { supports_table_and_stable, supports_table_and_stable },
        getmetatable = { supports_stable },
    ...
    for func_name, func_cfg in pairs(funcs_to_test) do
        ...
        while not finish do
            ...
            local expected_type
            if first_wrond_arg then
                expected_type = func_cfg[first_wrond_arg] == supports_stable and "effil.table" or "table"
            end
            local testcase_name = "check_types_" .. func_name ..
                (first_wrond_arg == nil and "_valid" or "_invalid")
            test.metatable_api[testcase_name] = template_test_function
            test.metatable_api[testcase_name](func_name, first_wrond_arg, expected_type, table.unpack(args))
...
--test.metatable_api.tear_down = function (metatable)
    collectgarbage()
    effil.gc.collect()
    test.equal(effil.gc.count(), 1)
--end
